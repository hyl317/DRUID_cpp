    ...
    FileOrGZ<FILE *> &logFile, double tot_genome, double bkg_sharing, int maxDeg)
{
    ...
    for(int i = 0; i < numThread; i++){
        std::thread t(processPairs, i, numThread, std::ref(pedigree), std::ref(comp_map), std::ref(allsegs), std::ref(snpmap), std::ref(*updated_results[i]), tot_genome, bkg_sharing, maxDeg);
